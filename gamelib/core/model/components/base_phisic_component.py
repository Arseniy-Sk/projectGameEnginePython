...
from gamelib.core.model.phisics.base_phisic_logic import impulse, falling
...
class BasePhisicComponent(Component):
    ...
    def set_position(self, dx, dy):
        if not self.obj or not hasattr(self.obj, 'update_position'):
            return
        
        current_pos = self.obj.get_position()
        self.obj.update_position(current_pos[0] + dx, current_pos[1] + dy)
    ...
    def stop_impulse(self):
        if not self.current_impulse_task or not self.obj or not self.obj.canvas:
            return
